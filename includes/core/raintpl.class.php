...
class RainTPL {
	 ...
	 * @var array
	 */
	static $path_replace_list = array (
			'a',
			'img',
			'link',
			'script',
...
$cache_id = null; // identify only one cache
	 ...
	 * Assign variable
	 * eg.
	 * $t->assign('name','mickey');
	 *
	 * @param mixed $variable_name
	 *        	Name of template variable or associative array name/value
	 * @param mixed $value
	 *        	value assigned to this variable. Not set if variable_name is an associative array
	 */
	 ...
	 * Draw the template
	 * eg.
	 * $html = $tpl->draw( 'demo', TRUE ); // return template in string
	 * or $tpl->draw( $tpl_name ); // echo the template
	 *
	 * @param string $tpl_name
	 *        	template to load
	 * @param boolean $return_string
	 *        	true=return a string, false=echo the template
	 * @return string
	 */
	 ...
	 *
	 * @param string $tpl_name
	 *        	Name of template (set the same of draw)
	 * @param int $expiration_time
	 *        	Set after how many seconds the cache expire and must be regenerated
	...
	/**
	 * execute stripslaches() on the xml block.
	 * Invoqued by preg_replace_callback function below
	 * 
	 * @access protected
	 */
	...
	protected function compileFile($tpl_basename, $tpl_basedir, $tpl_filename, $cache_dir, $compiled_filename) {
		...
		// disable php tag
		if (! self::$php_enabled)
			$template_code = str_replace ( array (
					"<?",
					"?>" 
			), array (
					"&lt;?",
			...
			// xml re-substitution
		$template_code = preg_replace_callback ( "/##XML(.*?)XML##/s", array (
				$this,
				'xml_reSubstitution' 
		), $template_code );
		...
		while ( $html = array_shift ( $parsed_code ) ) {
			...
			elseif (preg_match ( '/\{include="([^"]*)"(?: cache="([^"]*)"){0,1}\}/', $html, $code )) {
				...
				if (isset ( $code [2] )) {
					...
					// dynamic include
					$compiled_code .= '<?php $tpl = new ' . get_class ( $this ) . ';' . '$tpl_dir_temp = self::$tpl_dir;' . '$tpl->assign( $this->var );' . (! $loop_level ? null : '$tpl->assign( "key", $key' . $loop_level . ' ); $tpl->assign( "value", $value' . $loop_level . ' );') . '$tpl->draw( dirname("' . $include_var . '") . ( substr("' . $include_var . '",-1,1) != "/" ? "/" : "" ) . basename("' . $include_var . '") );' . '?>';
				}
			}			
	...
	/**
	 * Reduce a path, eg.
	 * www/library/../filepath//file => www/filepath/file
	 * 
	 * @param type $path        	
	 * @return type
	 */
	...
	protected function path_replace($html, $tpl_basedir) {
		if (self::$path_replace) {
			...
			if (in_array ( "script", self::$path_replace_list )) {
				$exp = array_merge ( $exp, array (
						'/<script(.*?)src=(?:")(http|https)\:\/\/([^"]+?)(?:")/i',
						'/<script(.*?)src=(?:")([^"]+?)#(?:")/i',
						'/<script(.*?)src="(.*?)"/',
						'/<script(.*?)src=(?:\@)([^"]+?)(?:\@)/i' 
				) );
			...
			if (in_array ( "a", self::$path_replace_list )) {
				$exp = array_merge ( $exp, array (
						'/<a(.*?)href=(?:")(http\:\/\/|https\:\/\/|javascript:)([^"]+?)(?:")/i',
						'/<a(.*?)href="(.*?)"/',
						'/<a(.*?)href=(?:\@)([^"]+?)(?:\@)/i' 
				) );
				$sub = array_merge ( $sub, array (
						'<a$1href=@$2$3@',
						'<a$1href="' . self::$base_url . '$2"',
						'<a$1href="$2"' 
				) );
			}
